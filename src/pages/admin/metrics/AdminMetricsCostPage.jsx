...
export default function AdminMetricsCostPage({ isAuthed, getAccessToken, apiBase, setActivePage }) {
  ...
  return (
    ...
    >
      ...
      {/* Biographies (free tier) breakout */}
      <ChartFrame
        eyebrow="Biographies · free tier"
        title="Biography chat cost"
        description="Family members chatting with a shared biography. Attributed to the family member's account, never the interviewee; free to users, so the platform absorbs this spend."
        exportName="cost-biographies"
      >
        <div style={{ display: "flex", gap: 28, flexWrap: "wrap", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--ink-soft)", letterSpacing: "0.08em" }}>
          <div>
            <div style={{ textTransform: "uppercase", marginBottom: 4 }}>Biography cost</div>
            <strong style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 20 }}>
              {fmtUsd(maybeObfuscateNumber(data?.biography?.total_cost || 0, { demoMode, isCount: false }))}
            </strong>
          </div>
          <div>
            <div style={{ textTransform: "uppercase", marginBottom: 4 }}>Chats</div>
            <strong style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 20 }}>
              {fmtTokens(maybeObfuscateNumber(data?.biography?.calls || 0, { demoMode }))}
            </strong>
          </div>
          <div>
            <div style={{ textTransform: "uppercase", marginBottom: 4 }}>Tokens</div>
            <strong style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 20 }}>
              {fmtTokens(maybeObfuscateNumber(data?.biography?.total_tokens || 0, { demoMode }))}
            </strong>
          </div>
          <div>
            <div style={{ textTransform: "uppercase", marginBottom: 4 }}>Family members</div>
            <strong style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 20 }}>
              {fmtTokens(maybeObfuscateNumber(data?.biography?.distinct_listeners || 0, { demoMode }))}
            </strong>
          </div>
          <div>
            <div style={{ textTransform: "uppercase", marginBottom: 4 }}>Biographies read</div>
            <strong style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 20 }}>
              {fmtTokens(maybeObfuscateNumber(data?.biography?.distinct_personas || 0, { demoMode }))}
            </strong>
          </div>
          <div>
            <div style={{ textTransform: "uppercase", marginBottom: 4 }}>Share of total</div>
            <strong style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 20 }}>
              {totalCost > 0 ? `${(((data?.biography?.total_cost || 0) / totalCost) * 100).toFixed(1)}%` : "—"}
            </strong>
          </div>
        </div>
      </ChartFrame>
